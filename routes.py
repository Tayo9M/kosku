...
def dashboard():
    ...
    if current_user.is_admin:
        ...
        # Get occupancy data (all properties) based on current month's occupancy records
        total_rooms = Room.query.count()
        
        # Count occupied rooms based on OccupancyRecord for current month
        current_month = datetime.now().strftime('%Y-%m')
        occupied_rooms = db.session.query(OccupancyRecord).join(
            Room, OccupancyRecord.room_id == Room.id
        ).filter(
            OccupancyRecord.month == current_month,
        ...
        # Get occupancy data (filtered by accessible properties) based on current month's occupancy records
        total_rooms = Room.query.filter(Room.property_id.in_(accessible_property_ids)).count()
        
        # Count occupied rooms based on OccupancyRecord for current month, filtered by property access
        current_month = datetime.now().strftime('%Y-%m')
        occupied_rooms = db.session.query(OccupancyRecord).join(
            Room, OccupancyRecord.room_id == Room.id
        ).filter(
...
@app.route('/edit_occupancy/<int:record_id>', methods=['GET', 'POST'])
@login_required
@admin_required  # Hanya Admin yang dapat mengedit data
def edit_occupancy(record_id):
    ...
    if request.method == 'POST':
        ...
        # Simpan status pembayaran awal sebelum diubah
        original_payment_status = record.payment_status
        
        # Update record data
        record.is_occupied = is_occupied
        record.tenant_name = tenant_name if is_occupied else None
        record.notes = notes
        old_payment_status = record.payment_status
        record.payment_status = payment_status if is_occupied else 'unpaid'
        record.payment_due_date = payment_due_date
        record.payment_months = payment_months if is_occupied else 1
        record.payment_date = payment_date if payment_status == 'paid' else None
        
        # Update room status
        room.status = 'occupied' if is_occupied else 'available'
        
        # Jika status berubah menjadi "paid" (sudah bayar) dan sebelumnya bukan "paid"
        # atau jika jumlah bulan pembayaran berubah, perbarui/buat data keuangan
        financial_updated = False
        if is_occupied and payment_status == 'paid' and original_payment_status != 'paid':
            # Hitung total pembayaran
            total_payment = room.monthly_rate * payment_months
            
            # Cari data keuangan terkait dengan data hunian ini
            existing_financial = FinancialRecord.query.filter(
                FinancialRecord.property_id == room.property_id,
                FinancialRecord.transaction_type == 'income',
                FinancialRecord.category == 'Sewa',
                FinancialRecord.description.like(f'%kamar {room.number}%{tenant_name}%{record.month}%')
            ).first()
            
            if existing_financial:
                # Jika sudah ada data keuangan, update jumlahnya
                existing_financial.amount = total_payment
                existing_financial.transaction_date = payment_date or datetime.now().date()
                existing_financial.description = f"Pembayaran sewa kamar {room.number} oleh {tenant_name} untuk {payment_months} bulan mulai {record.month}"
                db.session.add(existing_financial)
                financial_updated = True
            else:
                # Jika belum ada data keuangan, buat baru
                new_financial = FinancialRecord(
                    property_id=room.property_id,
                    transaction_date=payment_date or datetime.now().date(),
                    amount=total_payment,
                    transaction_type='income',
                    category='Sewa',
                    description=f"Pembayaran sewa kamar {room.number} oleh {tenant_name} untuk {payment_months} bulan mulai {record.month}",
                    created_by=current_user.id
                )
        ...
        if financial_updated:
            flash('Data hunian dan data keuangan terkait berhasil diperbarui', 'success')
        else:
            flash('Data hunian berhasil diperbarui', 'success')
...
def delete_occupancy(record_id):
    ...
    # Cari data keuangan terkait dengan data hunian ini (catatan pembayaran sewa)
    # Kriteria: tipe transaksi 'income', kategori 'Sewa', dan deskripsi berisi nomor kamar dan nama penyewa
    related_financial_records = FinancialRecord.query.filter(
        FinancialRecord.transaction_type == 'income',
        FinancialRecord.category == 'Sewa',
        FinancialRecord.description.like(f'%kamar {room.number}%{record.tenant_name}%'),
        FinancialRecord.property_id == room.property_id
    ).all()
    
    # Hapus data keuangan terkait
    financial_records_deleted = 0
    for fin_record in related_financial_records:
        db.session.delete(fin_record)
        financial_records_deleted += 1
    
    # Hapus data hunian
    db.session.delete(record)
    
    # Perbarui status kamar berdasarkan data hunian terbaru untuk bulan ini
    current_month = datetime.now().strftime('%Y-%m')
    latest_occupancy = OccupancyRecord.query.filter(
        OccupancyRecord.room_id == room.id,
        OccupancyRecord.month == current_month,
        OccupancyRecord.id != record.id  # Exclude record being deleted
    ).order_by(OccupancyRecord.created_at.desc()).first()
    
    if latest_occupancy and latest_occupancy.is_occupied:
    ...
    # Beri tahu pengguna tentang data apa saja yang dihapus
    if financial_records_deleted > 0:
        flash(f'Data hunian berhasil dihapus, {financial_records_deleted} data keuangan terkait dihapus, dan status kamar diperbarui', 'success')
    else:
        flash('Data hunian berhasil dihapus dan status kamar diperbarui', 'success')
    return redirect(url_for('manage_occupancy'))
...
def room_stats():
    ...
    return redirect(url_for('room_stats_js', **query_params))
